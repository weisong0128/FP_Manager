  ...
  var app = {
    ...
    uploadFile: function (callback) {
      ...
      self.upload.bind('FilesAdded', function (uploader, files) {
            ...
            if (!sqlReg.test(files[0].name)) {
              publicApp._toastDialog("文件名格式错误", {"intent": "danger", "position": "top_center"});
              return false;
            }

            var progress = '<div class="upload-wrap uploadScriptInfo" data-wrap-id="' + files[0].id + '">' +
            ...
            $(".upload-wrap").hasClass("uploadScriptInfo") ? $(".uploadScriptInfo").replaceWith($progress) : $("#uploadScript-module").append($progress);
            uploader.start();
          }
      );
