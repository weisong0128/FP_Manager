...
// 初始化表格及分页
$.ajax({
  url: basePath + '/project/result',
  type: 'GET',
  dataType: 'json',
  data: filter
}).then(function (res) {
  table = $('.table').DataTable({
    data: res.data.errResult,
    // 禁止排序
    ordering: false,
    lengthChange: false,
    searching: false,
    ...
    info: false,
    columns: [
      { data: 'dateStr' ,width: 80},
      { data: 'tag' , width: 100},
      { data: 'alterTag' , width: 100},
      { data: 'sqlResult', width: 500, className: 'break-all' },
      { data: 'pjName' , width: 60},
      { data: 'pjLocation' , width: 100}
    ],
  });
...
function _renderTable() {
  $.ajax({
    ...
    table.rows().remove();
    table.rows.add(res.data.errResult);
    table.draw();
    $('.page-wrapper').pagination({
      totalrows: res.data.page.totalRows,
      pagesize: res.data.page.pageSize,
